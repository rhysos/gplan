...
interface FlowerFormProps {
  ...
  onSubmit: (flower: { name: string; spacing: number; image_url: string }) => Promise<void>
  isLoading?: boolean
  initialFlower?: { name: string; spacing: number; image_url: string } | null
}
...
export function FlowerForm({ open, onOpenChange, onSubmit, isLoading = false, initialFlower = null }: FlowerFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    await onSubmit({ name, spacing, image_url: imageUrl })
    resetForm()
  }
          ...
          <div className="space-y-2">
            ...
            {imageUrl && (
              <div className="mt-4">
                ...
                <CloudinaryImage src={imageUrl} alt="Flower preview" width={100} height={100} className="rounded-md" />
              </div>
            )}
          </div>
